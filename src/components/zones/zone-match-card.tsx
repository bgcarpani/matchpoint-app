...
import type { ZoneMatchRow } from '@/components/zones/zone-manager'

const courtTypeLabel = (type: CourtOption['type']) =>
  type === 'indoor' ? 'Techada' : 'Aire libre'

/**
 * Tarjeta compacta de un partido de zona (estilo marcador): las dos parejas
 * apiladas con su casillero de resultado a la derecha, y un pie discreto con la
 * cancha y el botón Guardar. Reemplaza el apilado alto de v1 (título + dropdown
 * full-width + cajas grandes + botón grande) sin cambiar la lógica de guardado
 * ni de asignación de cancha.
 */
export function ZoneMatchCard({
  match,
  courts,
  scoringMode,
  gamesPerSet,
  ...
  const showInputs = canRecordResults && (!finished || editing)
  ...
  // Cada columna alinea verticalmente el casillero del team1 (arriba) y team2.
  const cols =
    scoringMode === 'games'
      ? [0]
    ...
    <div className="rounded-lg border border-border bg-secondary">
      ...
      <div className="flex items-stretch gap-3 px-3 py-2">
        <div className="min-w-0 flex-1 space-y-1">
          <PairName label={match.team1Label} highlight={match.winner === 'team1'} />
          <PairName label={match.team2Label} highlight={match.winner === 'team2'} />
        </div>

        {(canRecordResults || (finished && match.scoreDetail)) && (
        ...
        <div className="no-print flex items-center gap-1.5">
          {canManage && isManual && (
            <button
              ...
              className="rounded-md border border-border px-2 py-0.5 text-xs text-muted-foreground transition hover:bg-accent hover:text-foreground disabled:opacity-50"
            >
              Borrar
            </button>
          )}
...
function PairName({ label, highlight }: { label: string; highlight: boolean }) {
  return (
