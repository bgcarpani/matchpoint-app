...
import type { ChampionStyle } from '@/lib/og/champion-story'

/**
 * Flujo de compartir la historia (Instagram) del campeón en DOS pasos:
 ...
 *     comparte. Así el panel "Diseño de la historia" no ocupa espacio hasta que se
 *     lo pide, y el estilo elegido (`?style=`) llega siempre al PNG compartido.
 *
 ...
 */
const STYLES: { value: ChampionStyle; label: string }[] = [
  { value: 'a', label: 'Marquesina' },
  { value: 'b', label: 'Sello' },
  { value: 'c', label: 'Editorial' },
]

export function ChampionStoryShare({
  storyUrl,
  url,
  text,
}: {
  ...
  storyUrl: string
  /** URL pública del torneo (para copiar al portapapeles / sticker de Enlace). */
  url: string
  /** Texto de acompañamiento para el share sheet. */
  text: string
}) {
  const [open, setOpen] = useState(false)
  const [style, setStyle] = useState<ChampionStyle>('a')
  const [busy, setBusy] = useState(false)
  const [hint, setHint] = useState<string | null>(null)

  const styledUrl = `${storyUrl}?style=${style}`
